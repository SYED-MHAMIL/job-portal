...
export default function    AdminCard({ jobTitle, jobDescription ,qualification,jobType,address ,companyName,companyLogo,salary,hold,deleted,jobID}: JobCardType) {
...
<div className="break-words whitespace-normal  pr-4 ">
  ...
  <div tabIndex={0} role="button" className=" flex text-center justify-center items-center text-white bg-blue-600 hover:bg-blue-700 rounded-full py-2 pl-3 px-2">action<IoIosArrowDown />
  </div>
  <ul tabIndex={0} className="dropdown-content menu bg-base-100 rounded-box z-[1] w-52 p-2 shadow">
    <li className="bg-gray-100 rounded-md" onClick={holdStatus}><a>{hold ? "Publish" : "Hold"}</a></li>
    <li onClick={deletedStatus}><a>{deleted ? "Recover" : "Delete"}</a></li>
  </ul>
</div>
