...
export default function SeeApplications() {
 ...
 useEffect(()=>{
  ...
  const fetchData = async () => {
    ...
    try {
      const unsub = onSnapshot(q, async (snaps) => {
        const todos = snaps.docs.map(async (docDataSnap) => {
          const jobData = docDataSnap.data();
          const companyUid = docDataSnap.data().comapnyUid;
  ...
  return (
    ...
    <h1 className=" text-2xl text-center mt-5 font-extrabold leading-none tracking-tight text-gray-900 md:text-5xl lg:text-4xl dark:text-black">
        Jobs{" "}
      ...
      <div className="flex mt-14 m-2  justify-center min-h-screen bg-center bg-cover m">
        ...
        <div className="max-w-3xl w-full mx-auto z-10">
              ...
              fetchAllJobs.map(
                (
                  ...
                  <JobsApplication
                    ...
                    companyName={companyAllData.name}
                    jobTitle={companyOffer.jobTitle}
                  />
                )
              )}
          </div>
          ...
          status="404"
          title="404"
          subTitle="Sorry, You did not apply for the job  yet"
