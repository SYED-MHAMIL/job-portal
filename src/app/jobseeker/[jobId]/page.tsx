...
export default function JobId({ params: { jobId } }: paramsType) {
  ...
  const fetchData = async () => {
    ...
    console.log(jobData);
    const CompanyUidForm = jobData?.uid;
    setComapnyUid(CompanyUidForm);
  };

  const SubmitAppliaction = async () => {
    const jobseekerUid = auth.currentUser?.uid;
  ...
  return (
    <>
      <div className="flex items-center  mt-40 pt-0 flex-col w-full h-screen">
        ...
        <div className="relative  lg:w-1/2 min-w-[200px] ">
          <textarea value={applicationText}  onChange={(e)=>{
                        setApplicationText(e.target.value)
          }}
            className="peer h-full min-h-[100px] w-full resize-none rounded-[7px] border-1 border-blue-500 bg-transparent px-3 py-2.5 font-sans text-sm font-normal text-blue-gray-700 outline outline-0 transition-all placeholder-shown:border placeholder-shown:border-blue-gray-200 placeholder-shown:border-t-blue-gray-200 focus:border-2 focus:border-blue-600 focus:border-t-transparent focus:outline-0 disabled:resize-none disabled:border-0 disabled:bg-blue-gray-50"
            placeholder=" "
          ></textarea>
          ...
          <button
            disabled={isDisabled}
            className="mr-2 my-1 uppercase tracking-wider px-2 w-full text-blue-600 border-blue-600 hover:bg-blue-600 hover:text-white border text-sm font-semibold rounded py-1 transition transform duration-500 cursor-pointer"
            onClick={SubmitAppliaction}
          >
            Submit Appliaction
          </button>
        </div>
