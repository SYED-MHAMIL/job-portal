...
:
      fetchAllJobs.length > 0 &&
        fetchAllJobs.map(({ jobTitle, jobDescription,jobType ,address,companyAllData,salaryRange,docId,qualification}) => (
          <JobseekerCard key={docId} jobTitle={jobTitle} jobDescription={jobDescription} jobType={jobType} address={address} companyName={companyAllData.name} companyLogo={companyAllData.logo} salary={salaryRange} jobID={docId} qualification={qualification}/>
        ))}
        </div>
